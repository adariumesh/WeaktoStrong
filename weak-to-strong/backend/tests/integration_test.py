...
class TestDataAnalysisIntegration:
    ...
    @pytest.mark.asyncio
    async def test_data_analysis_execution_service_direct(
        ...
        if not track:
            pytest.skip(
                "Data Analysis track not found - run seed_data_challenges.py first"
            )
        ...
        if not challenge:
            pytest.skip(
                "No Data Analysis challenges found - run seed_data_challenges.py first"
            )
...
"""
        ...
        result = await execution_service.execute_challenge(
            execution_request, db_session
        )
        ...
        assert len(result.errors) == 0 or not any(
            "Docker" in str(error) for error in result.errors
        ...
        if result.success:
            ...
            print(
                f"⚠️  Challenge execution completed but validation failed: {result.errors}"
            )

    @pytest.mark.asyncio
    async def test_data_analysis_api_endpoint(
        self,
        async_client: AsyncClient,
        test_user: User,
        data_analysis_challenge: Challenge,
    ):
        """
...
"""
            ...
            print(
                f"  Result: {'✅ PASS' if result.success else '❌ FAIL'} ({result.score}/{result.max_score})"
            )
            if result.errors:
                print(f"  Errors: {result.errors}")
        ...
        execution_errors = [
            r for r in results if any("Docker" in str(e) for e in r.errors)
        ]
        assert len(execution_errors) < len(
            results
        ), "All challenges failed with Docker errors"
    ...
    async def test_validation_rules_parsing(self, db_session: AsyncSession):
        ...
        challenge_result = await db_session.execute(
            select(Challenge)
            .where(
                Challenge.track_id == track.id, Challenge.validation_rules.isnot(None)
            )
            .limit(1)
        )
        challenge = challenge_result.scalar_one_or_none()
