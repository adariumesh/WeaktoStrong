...
class DataRunner:
    ...
    async def execute_data_challenge(
        self, user_id: str, code: str, challenge: DataChallenge
    ) -> DataExecutionResult:
        """
        ...
        logger.info(
            f"Executing data challenge {challenge.challenge_id} for user {user_id}"
        )
    ...
    def _prepare_execution_config(
        self, user_code: str, challenge: DataChallenge
    ) -> Dict:
        """
        ...
        if challenge.initial_code:
            execution_script.extend(
                ["# Initial setup code", challenge.initial_code, ""]
            )

        # Add user's analysis code
        execution_script.extend(["# User's data analysis code", user_code, ""])
    ...
    def _process_sandbox_result(
        ...
        return DataExecutionResult(
            ...
            errors=(
                [container_result.get("error")] if container_result.get("error") else []
            ),
    ...
    def _apply_vibe_coder_scoring(
        self, result: DataExecutionResult, challenge: DataChallenge
    ) -> DataExecutionResult:
        """
